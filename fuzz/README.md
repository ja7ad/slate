...
```sh
cargo +nightly fuzz run fuzz_record_decode fuzz/artifacts/fuzz_record_decode/crash-<hash>
```
...
Seed the corpus from real data when you have it: `fuzz/corpus/<target>/` picks up any files you drop in, and a genuine `data.bin` from a test database is a far better starting point than random bytes.
