...
- **Stack Budget**: `slate_open` needs at least **52 KiB of stack** (measured by running it on pthreads with bounded stack sizes: SIGSEGV at 48 KiB, clean at 52 KiB; every other operation fits inside the same budget, so mount is the peak). A normal OS thread has megabytes and never notices, but any runtime that gives a coroutine, green thread, or fiber a smaller stack will take a **SIGSEGV on the first call into the engine, with no other symptom**. This is what made `tinygo test` crash in the first test that merely opened and closed a database while plain `go test` passed: TinyGo runs each test on a goroutine whose default stack is far below 52 KiB, whereas Go's goroutines grow on demand. If your host language has a small or fixed coroutine stack, either raise it (`tinygo -stack-size=1MB`) or call the engine from a full-sized thread.
