...
use slate_core::log::{HeadState, Log, Sealer};
use slate_crypto::keys::{DeviceKey, KeySet};
use slate_crypto::sealer::CryptoSealer;
use slate_erasure::matrix::cauchy_row;
use slate_erasure::reconstruct::{reconstruct, BlockSet};
use slate_erasure::{PAGE_SIZE, RS_K, RS_M, RS_N};
...
fn test_reed_solomon_database_recovery() {
    ...
    let mut chain = slate_core::chain::Chain::anchor(1, &[0u8; 32]);
    let mut log = Log::<'_, slate_sim::SimFlash>::new(
        &mut log_buf,
        HeadState {
            seg_seq: 1,
            write_offset: 0,
    ...
    for (exp_key, exp_val) in expected {
        ...
        let hdr = slate_core::record::RecordHeader::decode(&hdr_bytes).unwrap();
