...
pub struct Db {
    inner: Mutex<Option<OwnedEngine>>,
}
...
impl Drop for Db {
    ...
    fn drop(&mut self) {
        if let Ok(mut inner) = self.inner.lock() {
            if let Some(engine) = inner.as_mut() {
                let _ = engine.slate.commit();
            }
        }
    }
}
...
impl Db {
    pub fn open(
        ...
        let (engine_state, plain_len, replay_from, ckpt_seg_seq) =
            match slate_kv_core::epoch::mount(&mut flash, &mut counter, &mut sealer, ckpt_slice) {
                ...
                Err(MountError::FormatError) => {
                    ...
                    let seal_res = slate_kv_core::epoch::seal_epoch(
                        &mut st,
                        &mut flash,
                        &mut counter,
                        &mut sealer,
                    ...
                    if let Err(e) = seal_res {
                        return Err(Box::new((e.into(), flash, counter)));
                    }
                    (st, 0, data_base, 1)
                }
                Err(e) => return Err(Box::new((e.into(), flash, counter))),
            };
        ...
        Ok(Db {
            inner: Mutex::new(Some(OwnedEngine { slate, bufs })),
        })
    }
    ...
    pub fn commit(&self) -> Result<(), DbError> {
        ...
        inner.as_mut().unwrap().slate.commit()?;
        Ok(())
    }

    pub fn compact(&self) -> Result<(), DbError> {
        let mut inner = self.inner.lock().unwrap();
        inner.as_mut().unwrap().slate.compact()?;
        Ok(())
    }
    ...
    pub fn security_mode(&self) -> SecurityMode {
        let mut inner = self.inner.lock().unwrap();
        inner.as_mut().unwrap().slate.engine.security_mode
    }
    ...
    pub fn flash_mut<R, F: FnOnce(&mut crate::SimFlash) -> R>(&self, f: F) -> R {
        let mut inner = self.inner.lock().unwrap();
        f(&mut inner.as_mut().unwrap().slate.flash.0)
    }

    pub fn take_flash_and_counter(&mut self) -> (crate::SimFlash, crate::SimCounter) {
        let mut inner = self.inner.lock().unwrap();
        let engine = inner.take().unwrap();
        (engine.slate.flash.0, engine.slate.counter.0)
    }
}
